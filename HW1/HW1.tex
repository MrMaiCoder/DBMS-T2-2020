...
\begin{document}
	...
	$\Pi_{B}(R \bowtie S) = \Pi_{R \cup B} (\sigma_{R.B = S.B} (R \times B)) = \{B | B \in R \cap B \in S\}$ 
	
	For the right side, 
	$\Pi_{B}(R) \cap \Pi_{B}(S) = \{B | B \in R \cap B \in S\} $ \\
	...
	\includegraphics[width=\textwidth]{[DBMS]HW1.png} \\
	\part{2} \\
	...
	\begin{verbatim}
	SELECT ABS(compc.price - comlaptop.price) difference
	FROM computer compc
	INNER JOIN computer comlaptop ON
	    compc.maker = comlaptop.maker
	INNER JOIN pc ON 
    	pc.model = compc.model
	INNER JOIN laptop ON
    	laptop.model = comlaptop.model
	WHERE ABS(compc.price - comlaptop.price) < 100
	\end{verbatim}
